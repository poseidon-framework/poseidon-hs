...
main = do
    ...
    catch (runSafeT . usePoseidonLogger logMode $ runCmd logMode subcommand) (handler logMode)
    where
...
runCmd :: (MonadSafe m) => LogMode -> Subcommand -> PoseidonLogM m ()
