...
testGetJointGenotypeData = describe "Poseidon.Core.Package.getJointGenotypeData" $ do
    ...
    it "should correctly load genotype data without intersect" $ do
        ...
        jointDat <- runSafeT $ do
            jointProd <- getJointGenotypeData noLog False False True pacs Nothing
            P.toListM jointProd
    ...
    it "should correctly load genotype data with intersect" $ do
        pacs <- testLog $ mapM (readPoseidonPackage testPacReadOpts) pacFiles
        jointDat <- runSafeT $ do
            jointProd <- getJointGenotypeData noLog True True True pacs Nothing
            P.toListM jointProd
    ...
    it "should correctly load the right nr of SNPs with snpFile and no intersect" $ do
        pacs <- testLog $ mapM (readPoseidonPackage testPacReadOpts) pacFiles
        jointDat <- runSafeT $ do
            jointProd <- getJointGenotypeData noLog False False True pacs (Just "test/testDat/snpFile.snp")
            P.toListM jointProd
        length jointDat `shouldBe` 6
    it "should correctly load the right nr of SNPs with snpFile and intersect" $ do
        pacs <- testLog $ mapM (readPoseidonPackage testPacReadOpts) pacFiles
        jointDat <- runSafeT $ do
            jointProd <- getJointGenotypeData noLog True False True pacs (Just "test/testDat/snpFile.snp")
            P.toListM jointProd
        length jointDat `shouldBe` 4
    it "should fail with unordered SNP input file" $ do
        pacs <- testLog $ mapM (readPoseidonPackage testPacReadOpts) pacFiles
        let makeJointDat = runSafeT $ do
                jointProd <- getJointGenotypeData noLog False False True pacs (Just "test/testDat/snpFile_unordered.snp")
                P.toListM jointProd
    ...
    it "should skip incongruent alleles and allele flips" $ do
        ...
        jointDat <- runSafeT $ do
            jointProd <- getJointGenotypeData noLog True False True pacs Nothing
            P.toListM jointProd
        length jointDat `shouldBe` 6
