...
testPipelineChecksumupdate testDir checkFilePath = do
    ...
    let action = runChecksumupdate checksumOpts1 >> patchLastModified testDir ("Schiffels" </> "POSEIDON.yml")
    runAndChecksumFiles checkFilePath testDir action "checksumupdate" [
          "Schiffels" </> "POSEIDON.yml"
        ]
